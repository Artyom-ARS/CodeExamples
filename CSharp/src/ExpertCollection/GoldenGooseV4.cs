...
{
    public class GoldenGooseV4 : BaseExpert, IExpert
    {
        ...
        public GoldenGooseV4(IExpertParametersAdapter expertParametersAdapter, IBarsHistoryAdapter barsHistoryAdapter)
        {
            _expertParametersAdapter = expertParametersAdapter;
            _barsHistoryAdapter = barsHistoryAdapter;
        }
        public GoldenGooseV4()
        {
        ...
        {
            var parsedParams = _expertParametersAdapter.GetParameters<GgV4Parameters>(parameters);
        ...
        private IList<TradingCommand> CloseByStopLossIfNeeded(PriceTick priceTick, IReadOnlyList<ActiveOrder> activeOrders)
        {
        ...
        private IList<TradingCommand> CloseWithProfitIfNeeded(PriceTick priceTick, IReadOnlyList<ActiveOrder> activeOrders)
        {
            if (activeOrders.Count > 1)
            {
                ...
                {
                    //if (orders.Count > 3 && orders.Count <= _profitSequence)
                    //{
                    //    if (ShouldCloseProfitSequenceByStopLoss(priceTick, orders))
                    //    {
                    //        CloseAllOpenOrders(orders, commands);
                    //        commands.AddRange(TradeOpen(priceTick, Lots));
                    //        return commands;
        ...
        {
            var closeCommand1 = CloseByStopLossIfNeeded(priceTick, activeOrders);
